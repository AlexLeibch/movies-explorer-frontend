...
function Movies({isLogin}) {
    ...
    const filteredRenderMovies = React.useMemo(() => filterMovies(renderMovie), [isShortFilms, renderMovie])
    ...
    function handleMoreRenderMovie() {
        ...
        setRenderMovie(filteredMovies.slice(0, card + countClickMoreFilms * coefficientScreen()))
    ...
    function handleSearch(evt) {
        ...
        if(pathname=== '/movies') {
            ...
            filterMoviesByKeyWord(localStorage.getItem('moviesList') ? JSON.parse(localStorage.moviesList) : [])
            setVisibleMovie('movies__visibility')
            setVisibilityButton('')
            setIsPreloaderOpen('')
